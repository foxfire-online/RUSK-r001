...
int SENSORDELAY = 6000; // milliseconds (runs x1)
int EVENTSDELAY = 1000; // milliseconds (runs x9)
int SLEEP_DELAY = 15; // seconds (runs x1) - should get about 24 hours on 2100mAH - 0 to disable
...
void setPinsMode() {
    ...
    pinMode(SOILTEN, OUTPUT);
    pinMode(SOILT, INPUT);

    pinMode(SOILHEN, OUTPUT);
    pinMode(SOILH, INPUT);

    pinMode(WEATHEN, OUTPUT);

    pinMode(ACCELEN, OUTPUT);
}
...
void setup() {
    ...
    // Initialize Power Inputs and Calibration
    emon1.current(POWR1, 103.1);
    emon2.current(POWR2, 103.1);
    emon3.current(POWR3, 103.1);
...
void loop(void) {
    if (Spark.connected())
    {
        ...
        // Power up sensors
        digitalWrite(SOILTEN, HIGH);
        digitalWrite(SOILHEN, HIGH);
        digitalWrite(WEATHEN, HIGH);
        digitalWrite(ACCELEN, HIGH);
        ...
        if (SLEEP_DELAY > 0)
        {
            digitalWrite(SOILTEN, LOW);
            digitalWrite(SOILHEN, LOW);
            digitalWrite(WEATHEN, LOW);
            digitalWrite(ACCELEN, LOW);
